...
while (orgsimcounter<=orgsimmaxcount){
 ...
 # plot(sort (abund, decreasing=T),log= "y" , col="Red")
  ...
  grDevices::pdf(paste(orgsimcounter,"Ren1racs_rees_CWR.pdf"),
      width=5,
      height=4,
      pointsize=12)
  ...
  #Plot each renyi fit thing
  grDevices::pdf(paste(orgsimcounter,"Ren2racs_rees_CWR.pdf"),
      width=5,
      height=4,
      pointsize=12)
...
utils::write.table(storallloglik, file = "loglig_racs_rees_CWR.csv", sep = ",", append = F, quote = TRUE,
            col.names = F, row.names = FALSE)

utils::write.table(storalltheta, file = "theta_racs_rees_CWR.csv", sep = ",", append = F, quote = TRUE,
            col.names = F, row.names = FALSE)

utils::write.table(storalli, file = "i_loglik_racs_CWR.csv", sep = ",", append = F, quote = TRUE,
            col.names = F, row.names = FALSE)
